...
<div class="flex-container-row login-container">
    <form method="post" action="/auth/login">
        ...
        <c:if test="${ error_type != null }" >
            ${ error_type == "INVALID_USERNAME" ? "error-x" : "ok-tick" }
        </c:if>">
            <input type="text" class="text-input
                    <c:if test="${ error_type != null }" >
                        ${ error_type == "INVALID_USERNAME" ? "text-input-error" : "text-input-ok" }
                    </c:if>" name="username"
                   value="${ sessionScope["flash_login_username"] }" placeholder="用户名"/>
        </div>
        <div class="input-wrapper inline-input-wrapper
        <c:if test="${ error_type != null && error_type == \"INVALID_PASSWORD\" }" >
            ${ error_type == "INVALID_PASSWORD" ? "error-x" : "ok-tick" }
        </c:if>">
            <input type="password" class="text-input
            <c:if test="${ error_type != null && error_type == \"INVALID_PASSWORD\" }" >
                ${ error_type == "INVALID_PASSWORD" ? "text-input-error" : "text-input-ok" }
            </c:if>" name="password"
